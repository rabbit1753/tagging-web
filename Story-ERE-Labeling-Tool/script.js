...
var QA_selector = document.getElementById('QA_selector');
...
function click_previous_QA() {
    let qa_val = parseInt(QA_selector.value,10);
    
    if(qa_val - 1 >= 0){
        qa_val = qa_val - 1;
        QA_selector.value = qa_val.toString();
        console.log(QA_selector.value);
        QA_selected();
    }
    console.log("previous");
}

function click_next_QA() {
    let val = file_selector.value;
    let qa_val = parseInt(QA_selector.value,10);

    if(qa_val + 1 < stories_json[val].file_answer.length){
        qa_val = qa_val + 1;
        QA_selector.value = qa_val.toString();
        console.log(typeof(QA_selector.value));
        QA_selected();
    }
...
function click_commit() {
    ...
    for (let i = 0; i < taggings.length; i++) {
        if (taggings[i]['tab'] == "Entity") {
            filted_tag_entity.push(taggings[i]);
        }
        else if (taggings[i]['tab'] == "Relation"){
            filted_tag_relation.push(taggings[i]);
        }
        else{
            filted_tag_event.push(taggings[i]);
        }
    }
    ...
    for (let i = 0; i < filted_tag_entity.length; i++) {
        // i indicate i-th tag
        let temp = {
            "Entity_type": filted_tag_entity[i]['entity_type'],
            "Text": filted_tag_entity[i]['args'][0]["Text"],
            "Start": filted_tag_entity[i]['args'][0]["Start"],
            "End": filted_tag_entity[i]['args'][0]["End"]
        }
        mentions.push(temp);
    }
    let data = {
    ...
    // Event 的 tag 收集
    let mentions1 = [];
    let url1 = "http://140.115.54.59:8000/LabelResult/Event/" + user_id;

    for (let i = 0; i < filted_tag_event.length; i++) {
        // i indicate i-th tag
        let which_QA = filted_tag_event[i]['which_QA'];
        let event_type = filted_tag_event[i]['event_type'];
        let event_subtype = filted_tag_event[i]['event_subtype']
        let event_args = []
        for (let j = 0; j < filted_tag_event[i]['args'].length; j++) {
            // j indicate j-th argument in i-th event
            let arg = {
                "Arg_type": filted_tag_event[i]['args'][j]["Arg_type"],
                "Text": filted_tag_event[i]['args'][j]["Text"],
                "Start": filted_tag_event[i]['args'][j]["Start"],
                "End": filted_tag_event[i]['args'][j]["End"]
            }
            event_args.push(arg);
        }
        let single_event = {
        ...
        mentions1.push(single_event);
    }
    let data1 = {
        "Doc_name": stories_json[file_selector.value].file_name,
        "Event_mentions": mentions1
    }
    post_result_to_server(url1, data1);

    //relation 的 tag 收集
    let mentions2 = [];
    let url2 = "http://140.115.54.59:8000/LabelResult/Relation/" + user_id;

    for (let i = 0; i < filted_tag_relation.length; i++) {
        // i indicate i-th tag
        let which_QA = filted_tag_relation[i]['which_QA'];
        let relation_type = filted_tag_relation[i]['relation_type'];
        let relation_subtype = filted_tag_relation[i]['relation_subtype'];
        let relation_args = []
        for (let j = 0; j < filted_tag_relation[i]['args'].length; j++) {
            // j indicate j-th argument in i-th relation
            let arg = {
                "Arg_type": filted_tag_relation[i]['args'][j]["Arg_type"],
                "Text": filted_tag_relation[i]['args'][j]["Text"],
                "Start": filted_tag_relation[i]['args'][j]["Start"],
                "End": filted_tag_relation[i]['args'][j]["End"]
            }
        ...
        mentions2.push(single_event);
    }
    let data2 = {
        "Doc_name": stories_json[file_selector.value].file_name,
        "Relation_mentions": mentions2
    }
    post_result_to_server(url2, data2);

    window.alert("Commit 完成");
}
...
function click_Done() {
    ...
    console.log(start);
    console.log(elapsed);
    let url_t = "http://140.115.54.59:8000/LabelResult/Timestamp/" + user_id;
    let tmp = stories_json[file_selector.value].file_name + ',' + String(elapsed/1000);
    let data_t = {
        "Doc_name": "Timestamp.txt",
        "Timestamp_mentions": tmp
    };
    post_result_to_server(url_t, data_t);

    let which_QA = stories_json[file_selector.value].file_answer[QA_selector.value].file_name;
    if (args.length == 0) {
        return;
    }
    let dict;
...
function click_tag(wrapper) {
    ...
    if (current_tab == "Event") {
        ...
        let sel2_val = taggings[num]['relation_subtype'];
        for (let key in menu1_menu2) {
            if (menu1_menu2[key].includes(sel2_val)) {
                select1.value = key;
                select1_changed();
...
function relation_displayer(i) {
    ...
    let which_QA = document.createElement('span');
    which_QA.innerHTML = taggings[i]['which_QA'];
    tag_wrapper.appendChild(which_QA);
    ...
    for (let j = 0; j < taggings[i]['args'].length; j++) {
        let arg_wrapper = document.createElement('div');
        arg_wrapper.style.width = 'fit-content';
        // let QA_number = document.createElement('span');
        // QA_number.innerHTML = taggings[i]['args'][j]['which_QA'];
        let arg = document.createElement('span');
        arg.innerHTML = taggings[i]['args'][j]['Arg_type'];
        let text = document.createElement('span');
        text.innerHTML = taggings[i]['args'][j]['Text'];

        // arg_wrapper.appendChild(QA_number);
        arg_wrapper.appendChild(arg);
        arg_wrapper.appendChild(text);
        tag_wrapper.appendChild(arg_wrapper);
    }
...
function file_selected() {
    ...
    taggings = [];
    temp_tag = [];
    generate_QA_list(file_selector.value);
    refresh_tag_display();
    refresh_args_display();
}
...
function QA_selected() {
    ...
    let answer = document.getElementById('pair-A');
    let question = document.getElementById('pair-Q');
    let attribute = document.getElementById('attribute_text');
    let val = file_selector.value;
    let qa_val = QA_selector.value; // attribute 與 QA 共用 index
    ...
    // console.log(stories_json[val].file_answer);
    // console.log(QA_selector.value);
    let tok_answer = stories_json[val].file_answer[qa_val].answer;
    let tok_question = stories_json[val].file_question[qa_val].question;
    let tok_attribute = stories_json[val].file_attribute[qa_val].attribute;

    for (let i = 0; i < tok_answer.length; i++) {
        let spam_a = document.createElement('spam');
        spam_a.innerHTML = tok_answer[i];
        spam_a.dataset.value = i;
        answer.appendChild(spam_a);
    }
    for (let i = 0; i < tok_question.length; i++) {
        let spam_q = document.createElement('spam');
        spam_q.innerHTML = tok_question[i];
        spam_q.dataset.value = i;
        question.appendChild(spam_q);
    }
    for (let i = 0; i < tok_attribute.length; i++) {
        let spam_q = document.createElement('spam');
        spam_q.innerHTML = tok_attribute[i];
        spam_q.dataset.value = i;
        attribute.appendChild(spam_q);
    }
    // refresh_tag_display();
    // refresh_args_display();
}
...
function click_Check() {
    start = Date.now();
    let val = file_selector.value;
    let qa_val = QA_selector.value;
    display_story(stories_json[val].file_name, stories_json[val].file_content, 
        stories_json[val].file_answer[qa_val].answer, stories_json[val].file_question[qa_val].question, stories_json[val].file_attribute[qa_val].attribute, flag);

    //如果上一個選擇的故事與當前不同才重新抓取tag，否則會有重複顯示的問題
    if (previous_file != file_selector.value){
        get_tagged_tags(stories_json[val].file_name);
        previous_file = file_selector.value;
    }
    refresh_tag_display();
    refresh_args_display();
}
...
function load_previous_tag(tags) {
    ...
    for (let i = 0; i < tags.length; i++) {
        ...
        if (filter == "Entity") {
            ...
            let which_QA = tags[i]["label"]["which_QA"];
            ...
            for (let j = 0; j < tags[i]["label"]["Arguments"].length; j++) {
                // let QA_number = tags[i]["label"]["Arguments"][j]["which_QA"];
...
function get_tagged_tags(file_name) {
    // http://140.115.54.59:8000/StoryTag/0/Fairytale/Snow-man01.txt
    let url = 'http://140.115.54.59:8000/StoryTag/' + user_id + '/Fairytale/';
    url = url + file_name;
...
function generate_QA_list(file_index) {
    // var datalistOptions = document.getElementById('datalistOptions');
    while (QA_selector.firstChild) {
        QA_selector.removeChild(QA_selector.firstChild);
    }
    // QA_selected.value = 1;
    for (let i = 0; i < stories_json[file_index].file_answer.length; i++) {
        let option = document.createElement('option');
        option.text = stories_json[file_index].file_answer[i].file_name;
        option.value = i;
        QA_selector.appendChild(option);
    }
    console.log(QA_selector.value); 
}

// display story content at story display area
function display_story(f_title, f_cont, f_answer, f_question, f_attribute, flag) {
    ...
    let tok_answer = f_answer;
    let tok_question = f_question;
    let tok_attribute = f_attribute;
    ...
    if (flag == 0) {
        var tok_cont = tokenization_ch(f_cont);
    }
    else if (flag == 1) {
        var tok_cont = tokenization_en(f_cont);
    }

    // Question

    for (let i = 0; i < tok_cont.length; i++) {
        let spam_c = document.createElement('spam');
        spam_c.innerHTML = tok_cont[i];
        spam_c.dataset.value = i;
        content.appendChild(spam_c);
    }
    for (let i = 0; i < tok_answer.length; i++) {
        let spam_a = document.createElement('spam');
        spam_a.innerHTML = tok_answer[i];
...
var ERE_menu1 = {
    ...
    "Relation": ["ART (artifact)", "GEN-AFF (Gen-affiliation)", "METONYMY",
                 ...
                 "PHYS (physical)", "Coreference", "Unk", "Causal Effect", "Temporal"],
...
var menu1_menu2 = {
    ...
    "Causal Effect": ["X intent","X need","X attribute","Effect on X","X want","X reaction","Other reaction","Other want","Effect on other","Other"],
    "Temporal": ["isBefore","isAfter"],
};
...
var relation_rgb = {
    ...
    "Unk": { "R": 255, "G": 0, "B": 51 },
    "X intent": { "R": 132, "G": 200, "B": 174 },
    "X need": { "R": 132, "G": 200, "B": 100 },
    "X attribute": { "R": 132, "G": 51, "B": 174 },
    "Effect on X": { "R": 51, "G": 200, "B": 174 },
    "X want": { "R": 102, "G": 200, "B": 174 },
    "X reaction": { "R": 102, "G": 51, "B": 174 },
    "Other reaction": { "R": 132, "G": 200, "B": 51 },
    "Other want": { "R": 132, "G": 40, "B": 0 },
    "Effect on other": { "R": 153, "G": 51, "B": 51 },
    "Other" : { "R": 204, "G": 51, "B": 204 },
    "isAfter" : { "R": 77, "G": 100, "B": 255 },
    "isBefore" : { "R": 187, "G": 255, "B": 30 },
    "Temporal" : { "R": 170, "G": 30, "B": 255 }
}
